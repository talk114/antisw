...
Usage:
    ...
    curl -fsSL https://raw.githubusercontent.com/${REPO}/main/install.sh | VERSION=5.0.0 bash
...
build_download_url() {
    ...
    case "$PLATFORM" in
        linux)
            case "$PKG_EXT" in
                deb)
                    # Antigravity.Tools_5.0.0_amd64.deb or _arm64.deb
                    ...
                    ;;
                rpm)
                    # Antigravity.Tools-5.0.0-1.x86_64.rpm or -1.aarch64.rpm
                    ...
                    ;;
                AppImage)
                    # Antigravity.Tools_5.0.0_amd64.AppImage or _aarch64.AppImage
                    local appimage_arch
